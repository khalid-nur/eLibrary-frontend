...
const MessageItem = () => {
  const { data: messageData, isLoading, error } = useMessages(0, 3);
  ...
  const renderMessagesContent = () => {
    ...
    if (error) {
      ...
      return (
        <StatusMessage
          avatarIcon={
            <TbMessageOff className="text-red-400 animate-pulse" size={56} />
          }
    ...
    if (!messageData || messageData.content.length === 0) {
      return (
        <StatusMessage
          avatarIcon={
            <TbMessageOff className="text-slate-400 animate-pulse " size={56} />
          }
